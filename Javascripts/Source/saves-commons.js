...
function writeLoadSave2(loadorsave)
{
	...
	md.write(
		...
		"}" +
		"function ImportSaveGame() { showPopupWindowNowImport(); }" +
		"</script>"
	);
	...
	for (var i = 1; i < (bChatLeft.length + 1); i++)	{
		...
		if (s !== '') {
			...
			s += "<span style='color:" + pclr + "'>Turn</span>" +
				'(' + tn + ')</span></a></td><td' + clr + ' style="width:10%"><a href="javascript:deleteSaveGame(\'' + SAVE_BASE + (ci <= 0 ? (ci === 0 ? "'Auto'" : "'Auto" + Math.abs(ci) + "'") : ci)  + '\');writeLoadSave(\'' + loadorsave + '\',\'' + order + '\')"><span style="font-size:small">(delete)</span></a></td>';
			if (pth == 1 || pth == 2) {
				col2.push(s);
				idx2++;
			} else {
	...
	if (loadorsave === "load") {
		...
		addOptionLink(md, "Save to File", "bChatLeft=true;download(getSaveString('Export'),'" + SAVE_BASE + nTime + ".txt','text/plain')", "chatblock");
		addOptionLink(md, "Save All to File", "download(getAllSaveString(bChatLeft),'" + SAVE_BASE + "ALL.txt','text/plain')", "chatblock");
	...
	md.write(
		'</div><div id="lightexport" class="white_content" style="background-color:black;color:white;">' +
	...
	md.write('<div id="commentdiv" class="comment_content_trans' + (gameState.bCommentLL ? '_ll' : '') + '" onclick="bChatLeft = true;ClearComments();"></div><div id="fadeblack" class="black_overlay"></div>');
