...
function getPhoneNotes(stype)
{
	...
	if (stype === "notes1") {
		...
		findPerson("Kylie");
		if (per.checkFlag(1)) sp += "Kylie plays sports weekdays 12-2pm<br>";
		findPerson("MsTitus");
		if (per.isFreeSlave()) sp += "Ms. Titus is my willing slave<br>";
		if (checkPersonFlag("Bambi", 7)) sp += "Bambi is your night guard in the dungeon<br>";
		if (!isMurderPath() && isCharmedBy("Sofia")) s += "Angelica is busy working in the mornings from 8am but is free after midday.<br>";
