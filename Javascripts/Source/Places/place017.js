...
function ShowPlace17()
{
	...
	if (perSarah.place == 17 && perSarah.other == 1) {
		...
		if (perGates.other > 5 && perGates.other < 499 && perGates.checkFlag(2)) {
			...
			addLinkToPlace(md, "study the books in " + perGates.getPersonNameShort() + "'s library", '', '', '<img src="UI/books/occult' + String.fromCharCode(Math.floor(Math.random() * 16) + 97) + '.jpg" style="width:20%;float:right;margin-left:5px;margin-bottom:1em" alt="Occult Book">You read some of the occult works, occasionally asking questions of ' + perGates.getPersonNameShort() + '. Time flies by and an hour has passed before you know it', '', 'WaitHere(6);');
		}
