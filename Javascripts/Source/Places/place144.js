...
function ShowPlace144(stype)
{
	...
	startQuestions();
	if (sType === "") {
		if (isDay()) addLinkToPlace(md, "sit on a bench for a while and watch a game", '', '', 'You kill some time for an hour, watching a game being played', '', 'WaitHere(5);');
		else addLinkToPlace(md, "sit on a bench for a while", '', '', 'You kill some time for an hour,', '', 'WaitHere(5);');
	}
