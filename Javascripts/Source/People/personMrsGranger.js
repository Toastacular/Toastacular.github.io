...
function initialiseMrsGranger()
{
	...
	per.showEvent = function()
	{
		...
		if (Place == 269 && sType == "grangerpoolsex") {
			md = WritePlaceHeader(false, this.dress == "ShinyBlue" ? 'td-left-med' : '');
			...
			if (sType === "comfortable") {
				...
				if (!this.checkFlag(12) && perKate.place == 1 && (perKate.isCharmedBy("You") || perKate.checkFlag(22))) {
					...
					md.write(
						...
						'<p>You can see her reluctance to do anything with Kate around, maybe you should just talk more to her.</p>'
					);
	...
	per.showPersonChat = function(bGeneral, md)
	{
		...
		if (Place != 177 || sType !== "" || !this.isHere()) return;
		...
		if (clv > 0) {
			...
			if (clv > 1) addLinkToPlaceC(md, this.other > 3 ? 'tell Mrs. Granger "Let\'s Celebrate"' : 'tell Mrs. Granger to get comfortable', Place, 'type=comfortable');
			...
			if (this.other > 3 && clv == 1 && !this.checkFlag(7)) addQuestionC(md, '"Celebrate?"', "MrsGranger", 1000);
			if (clv == 1) {
			...
			if (this.checkFlag(13) && perKate.place == 1) {
				// Both here now
