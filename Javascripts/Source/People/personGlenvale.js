...
function initialiseGlenvale()
{
	...
	per.showPlaceImageRight = function(md)
	{
		...
		switch (Place) {			
		case 26:
			if (id >= 2) this.showPersonAnon("townspeople-wildranges" + id + ".jpg");
			break;
		case 63:
		...
		case 2:
			...
			break;
		case 195:
			if (id == 9) this.showPersonAnon("townspeople-generalstore-exh1.jpg");
		...
		case 239:
			if (id == 1) {
				...
				this.setFlag(1);
			} else if (id == 2) this.showPersonAnon("townspeople-museum1.jpg");
			break;
		case 318:
	...
	per.showEvent = function()
	{
		...
		// Church
			...
			if (!this.checkFlag(33)) this.place = 318.01;
				...
				if (bNudist) this.place = 318.51;
				else this.place = Place + (Math.floor(Math.random() * 4 + 1) / 100);
			}
		...
		// Streets
		else if (isDay() && (Place == 194 || Place == 238 || Place == 360 || Place == 94 || Place == 455 || Place == 2) && Math.random() < 0.15) {
			if (bNudist) this.place = Place + (Math.floor(Math.random() * 8 + 51) / 100);	// Busy streets
			else this.place = Place + (Math.floor(Math.random() * 8 + 1) / 100);	// Busy streets
		}
		// Wild Ranges
		...
		// Sports field
		else if (Place == 144 && wherePerson("Kylie") != 144 && isShopOpen(2) && Math.random() < 0.2) {
			if (bNudist) this.place = Place + (Math.floor(Math.random() * 4 + 51) / 100);
			else this.place = Place + (Math.floor(Math.random() * 3 + 1) / 100);
		}
		// Park Pathway
		else if (Place == 63 && isDay() && Math.random() < 0.1) {
			if (bNudist) this.place = Place + (Math.floor(Math.random() * 6 + 51) / 100);
			else this.place = Place + (Math.floor(Math.random() * 7 + 1) / 100);
		}
		// Hotel Pool
		else if (Place == 269 && isDay() && Math.random() < 0.1) {
			if (bNudist) this.place = Place + (Math.floor(Math.random() * 8 + 51) / 100);
			else this.place = Place + (Math.floor(Math.random() * 6 + 1) / 100);
		}
		// Laundromat
		else if (Place == 199 && Math.random() < 0.1) this.place = Place + (Math.floor(Math.random() * 4 + 1) / 100);
		// Gym
