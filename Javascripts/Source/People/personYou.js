...
function spendExperience(type)
{
	...
	if (type === undefined) {
		...
		//if (!perYou.checkFlag(18) && nShown < 3) { nShown++; addOptionLink(md, "defending yourself with magic", "spendExperience(2)", "bloodblock"); }
		if (isMurderPath()) {
		...
		/*
			...
			// Counterspell
			perYou.setFlag(18);
			md.write('<p>After some study the passage is clearer, you can see how to summon mana you have to block a spell cast on you. It requires you to have a substantial reserve of mana, at least 20 points.</p>');
		*/
...
function initialiseYou()
{
	...
	perYou.showEventPopup = function()
	{
		...
		if (sType == "dream8") {
			this.setFlag(45);
			showPopupWindow("Dreaming",
				"<img src='Images/Dreams/dream8.jpg' style='width:45%;float:right;margin-left:5px' alt='Dream'>" +
	...
	perYou.showEventSleep = function(wt, plc, s, param)
	{
		...
		// Deamon in the bath
		if (this.checkFlag(45) && isDemonFreed() && Math.random() < 0.2) {
	...
	perYou.addEndGamePregnancies = function(md)
	{
		...
		var state = sType === "endgame1logan" ? 0 : sType == "endgame1dianegrangers" ? 1 : sType == "endgame1rosssisters" ? 2 : sType == "endgame1beasleydidi" ? 3 : sType == "endgame1carolellie" ? 4 : sType == "endgame1abbymiku" ? 5 : sType == "endgame1alisonjenny" ? 6 : sType == "endgame1anitamelissa" ? 7 : sType == "endgame1townhall" ? 8 : 99;
	...
	perYou.showEventEndGame = function() 
	{
		...
		if (sType == "endgame1harem") {
			...
			case 6:
				...
				md.write(
					'<p>At one time you became suspicious that Diane White and Mt. Beasley may have some sort of association or relationship, some of the matters around the Book and the investigations were a little suspicious to you.</p>' +
		...
		if (sType == "endgame1rosssisters") {
			...
			addPlaceTitle(md, "A Definitely Contagious Lesson?");
		...
		if (sType == "endgame1alisonjenny") {
			...
			md = WritePlaceHeader();
			if (isCharmedBy("Jenny")) findPerson("Jenny").showPerson("pregnant" + (this.checkFlag(4) ? "-futa" : "") + ".jpg");
		...
		if (sType == "endgame1townhall") {
			...
			if (isCharmed("Emily")) {
				md.write("<p>You meet Emily in the break room, and she closes the door and explains that she is " + (rtot > 1 ? "also" : "") + " pregnant!</p>");
			}
		...
		if (sType == "endgame1final") {
			...
			hideSidebars();
			md = WritePlaceHeader();
			img = perYou.isBornMale() ? 'endgame1-final-male' : perYou.getPersonGender() == "woman" ? 'endgame1-final-female' : isExplicit(true) ? Math.random() < 0.5 ? 'Endings/Explicit/endgame1-final-futa1' : 'Explicit/endgame1-final-futa2': 'endgame1-final-futa1';
