...
{
	// Ms Jones
	addPerson("Ms. Jones", 0, "MsJones", '', false);
	...
	{
		// Futa reaction
		if (sType == "proveexperience" && !this.checkFlag(18) && perYou.isFuta() && !perYou.isBornMale()) {
			this.setFlag(18);
			showPopupWindow("Ms. Jones and Your Changes",
				this.addPersonString("jones8a.jpg", "height:max%", "right") +
		...
		if (Place == 145 && !this.checkFlag(5) && this.hoursCharmed() > 8 && isCharmedBy("MissLogan") && this.isHere()) {
			showPopupWindow("Ms Jones knows...",
				this.addPersonString("jones1a.jpg", "height:max%", "right") +
	...
	per.showEvent = function()
	{
		...
		if (sType == "msjonespool") {
			...
			md.write(
				'<p>Ms. Jones arrives and changes into her bikini, ready to serve you in any way you wish!</p>'
			);
			startQuestions();
		...
		if (Place == 269 && sType == "msjonespoolsex") {
			...
			if (isExplicit() && perYou.isMaleSex()) this.showPersonX("jones-pool-sex.gif");
		...
		if (this.checkFlag(3) && !this.checkFlag(4)) {
				...
				if (isDemonInTown()) {
					...
					if (isDemonGone()) {
						md.write(
							...
							'<p>This terrible news strike you hard, and you tell Ms. Jones about Leanne and how she has become a demonic thrall and being held at the Church pending the arrival of an exorcist. Ms. Jones looks almost disgusted,</p>' +
			...
			if (sType == "jones5a" || sType == "jones5b") {
				...
				md = WritePlaceHeader(false, sType == "jones3" ? "td-left-large" : "");
		...
		if (sType == "bj") {
			...
			md = WritePlaceHeader(false, isExplicit() && !perYou.isMaleSex() ? "td-left-large" : "");
			if (isExplicit()) {
				if (perYou.isMaleSex()) this.showPersonRandomX("jones8", 5);
				else this.showPersonRandomX("jones9", 3);
			} else if (perYou.isMaleSex()) this.showPersonRandom("jones8", 2);
	...
	per.showDancing = function()
	{
		...
		if (ap) md.write('<p>Ms. Jone is wearning a sot os stylised gangster outfit and some a skilled strip-tease. She is very focused and an excellent stripper but you are briefly distracted by the thought that she could of dressed this way during the actual era she is dressed for...</p>');
		else md.write('<p>Ms. Jone is wearning a costume, a fantasy take on Catwoman, then again is any Catwoman costume worn for anthing else. She is an excellent dancer and can really handle that whip!</p>');
	...
	per.showPersonChat = function(bGeneral, md)
	{
		if (Place == 145 && this.isHere()) { 
			...
			if (perYou.isQuestStarted(5) && perLeanne.place == 382 && !perLeanne.checkFlag(8)) addLinkToPlaceC(md, 'ask Ms. Jones about saving Leanne', 145, 'type=askleanne');

			if (this.isCharmedBy()) {
				// Charmed only questions
				if (getQueryParam("post") === "") addLinkToPlace(md, 'let her prove her experience again', Place, 'type=proveexperience');
	...
	per.handleItem = function(no, cmd)
	{
		...
		if (no == 14 && cmd == 2) {
			...
			if (Place == 242 && this.isHere()) {
				...
				if (!this.isCharmed() && this.checkFlag(1)) CastCharmSpell("MsJones", 145, 1, "type=jones2");		//Charm Ms. Jones
	...
	per.isPhoneable = function() {
		// Can you call them?
		return checkPlaceFlag("Hotel", 11) && Place == 269 && this.isCharmedBy("You");
	};
	...
	per.getPersonSMS = function(id) {
		switch(id) {
			...
			case 41:
				// Miss Logan SMS
				return receiveSMS('frenchisfun', 'Ms Logan and I are getting ready for classes and we hope to see you there. (You still have some catching up to do, but If you are busy I will compile everything you need to know for later.)', 'jonessms2.jpg') +
