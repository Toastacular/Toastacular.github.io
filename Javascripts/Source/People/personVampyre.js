...
function initialiseVampyre()
{
	...
	per.showPlaceImageRight = function(md) {
		this.showPerson(Place == 46 ? "bedrooma.jpg" : "vamp5a.jpg", undefined, undefined, undefined, undefined, undefined, md);
		...
		for (var i = 0; i < arPeople.length - 2; i++) {
			...
			plc = p.whereNow();
			if (p.isCharmedBy("You") && plc == Place && p.health > 0 && !p.isVampyre() && Place != 282) {
	...
	per.showEvent = function()
	{
		...
		if (Place == 464) {
			if (sType == "bitealison") {
				...
				this.showPersonRandom("vampalison-lesbian");
