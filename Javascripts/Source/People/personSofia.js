...
function initialiseSofia()
{
	...
	per.getPersonAddress = function() { return this.checkFlag(1) ? 'Gate\'s Mansion Garage' : ''; };
	...
	per.showEventPopup = function()
	{
		...
		if (Place == 14 && this.isHere() && sType == "charm") {
			if (!perYou.isQuestStarted(7)) {
				if (isMurderPath()) {
					perYou.startQuest(7);
					showPopupWindow("Chauffeur",
						"<p><img src='Images/People/Sofia/sofia_ring.jpg' style='width:50%;float:right;margin-left:5px' alt='Failed!'>" +
					...
					showPopupWindow("Chauffeur",
						"<p><img src='Images/People/Sofia/sofia_ring.jpg' style='width:35%;float:right;margin-left:5px' alt='Failed!'>" +
			...
			} else addComments("<img src='Images/People/Sofia/sofia_ring.jpg' style='width:50%;float:right;margin:0 5px 1em 0' alt='Failed!'>Remember the ring!");
			return true;
		}
		...
		if (Place == 95 && this.checkFlag(11) && !this.checkFlag(12) && isCharmedBy("Angela")) {
			...
			showPopupWindow("Angela Helping",
				...
				'<p>"I know of a Sofia. She is a hispanic woman in her mind twenties. She was the personal driver to ' + perGates.getPersonNameShort() + ' Gates. Is she the one you are looking for?" she asks.</p>',
				"dispPlace(95)"
			);
			return true;
		}
	...
	per.showEvent = function()
	{
		...
		if (sType == "sofiapool") {
			...
			md.write(
				'<p>You ask Bambi to get Sofia a spare bikini and she quickly finds one for Sofia, who changes into it, ready to serve you in any way you wish!</p>'
			);
			startQuestions();
		...
		if (Place == 269 && sType == "sofiapoolsex") {
			...
			md.write(
				'<p>You ask Sofia for more with her privately, and she seductively removes most of her swimsuit and plays with her large breasts for your pleasure.</p>'
			);
			startQuestions();
		...
		if (Place == 110 && sType =='asksofia1') {
			...
			md.write(
				'<p>You ask Mayor Thomas about Sofia, the chauffeur at the Gates\'s Mansion, and she replies,</p>' +
				...
				'<p>Amused you ask her "Heh! You have anything interesting on me? and you smile to yourself, while Miss Thomas chuckles at your "original" joke and then replies,</p>' +
				'<p>"Oh yes! That you are the finest ' + (perYou.isBornMale() ? 'lad' : 'joker') + ' in the land and we should all appreciate that we know you. The greatest honor to us is when we can talk to you or you give us a task. Yep, it’s written here!" she smiles. Her smile suddenly wanes away, panic draws on her face.</p>' +
				'<p>You ask, "What’s wrong?" and she answers,</p>' +
				'<p>"' + perYou.getMaster() + '! I don’t- I don’t understand! There’s no one named Sofia in here! I’m so deeply sorry!" she still doesn’t give up, looking around, typing furiously, trying every possibility.</p>' +
				...
				'<p>You tell her "Nope, I thought you would be the one who can enlighten me…"</p>' +
				...
				'<p>"Don’t worry! You’ve done what you can. Now go back to your work.", you tell her.</p>'
			);
		...
		if (sType == "sofialimosex") {
			...
			md.write(
				'<p>You ask Sofia to join you in the backseat of the limo. Dutifully she joins you, her expression clearly shows she knows what her duties will be.</p>'
			);
		...
		if (sType == "sofialimofuck") {
			...
			md.write(
				'<p>You tell Sofia it is time for her to sit on your lap, in a somewhat intimate way.</p>'
			);
		...
		if (sType == "sofialimobj") {
			...
			md.write(
				'<p>You ask Sofia to help you get comfortable, and she kneels down ready to make you feel a lot more relaxed.</p>'
			);
		...
		if (sType == "sofialimotitfuck") {
			...
			md.write(
				'<p>You discuss parking with Sofia, and she gestures towards a garage. You instead gesture at her large breasts and suggest parking your cock <i>there</i>!</p>'
			);
		...
		if (sType == "sofiafuck") {
			...
			md.write(
				'<p>There is more than one way to go for a ride with Sofia, and some do not even need you to leave the office.</p>'
			);
		...
		if (sType == "sofiabj") {
			...
			md.write(
				'<p>You ask Sofia to give you a lift, she moves to get her keys and then sees you removing your clothing. She realises what you want her to lift and she steps over and kneels down ready to lift <i>you</i>.</p>'
			);
		...
		if (sType == "sofiatitfuck") {
			...
			md.write(
				'<p>You discuss parking with Sofia, and she gestures towards the garage. You instead gesture at her large breasts and suggest parking your cock <i>there</i>!</p>'
			);
	...
	per.showPersonChat = function(bGeneral, md)
	{
		...
		if (Place == 192 && sType === "") {
			if (this.checkFlag(1)) {
				if (!this.checkFlag(26) && !isCharmedBy("Sarah") && !isMurderPath()) {
					addQuestionR(md, '"What can you tell me about Sofia, the family chauffeur?"',
						'Sarah tells you, &quot;What a nosy little apprentice you are! She is just a simple woman who serves my uncle. You don’t have to worry about her, she is harmless and doesn’t speak our language.&quot;</p>' +
						'<p>You reply, &quot;She sure is memorable from the rest of us. I mean her dark skin and interesting looks…&quot;</p>' +
						'<p>Sarah answers &quot;Drop it ' + perYou.getPersonName() + '! Don’t you ever think about charming her or my uncle and I will make sure that you will never see daylight again. Sofia’s a friend and I will protect her from you if I have to.&quot;',
						"Sarah",
						"setPersonFlag(\\'Sofia\\',26)"
					);
				}
	...
	per.isPhoneable = function() { return perYou.isQuestComplete(7) && isOutside(); };
