...
function gotoPlace(plc, params, txt, ps, click)
{
	...
	if (stype.indexOf("threesome") != -1 || stype.indexOf("lesbian") != -1 || stype.indexOf("anal") != -1 || stype.indexOf("fuck") != -1 || stype.indexOf("sex") != -1 || stype.indexOf("bj") != -1 || stype.indexOf("lick") != -1 || stype.indexOf("strapon") != -1) {
...
function addLinkToPlaceI(desc, plc, param, txt, ps, js, psty) {
...
function addPlaceTitle(doc, tit, img, taxichance, notable, clr, lit, wid)
{
	...
	if (taxichance !== undefined && taxichance > 0) {
		...
		else doc.write('<p style="font-size:small;margin-bottom:-1px"><img draggable="false" src="Images/' +  (isBritish() ? "UK" : "US") + '/taxirank.png" style="margin: 5px 25px;" width="70" height="70" alt="Taxi Rank"><br>There is an <b>empty</b><br>taxi rank here</p>');
	}
...
function WritePlaceHeader(nt, alg, clr, ss, bgimg)
{
	...
	if (nt !== true) {
		...
		mdCache.write('<table id="maintable" class="table-main"><tr><td ' + alg + '>');
	}
	return mdCache;
}
...
function WritePlaceFooter(doc, credits, noitems, nopeople)
{
	...
	if (gameState.nRightBarState > 2 || !gameState.bInventoryPopup) doc.write('<div id="rightbar" class="sidebarright inventbar" style="position:fixed;top:0px;right:0;' + (gameState.nRightBarState == 2 ? 'overflow-y:hidden;' : '') + 'width:' + rm + '">' + (gameState.nRightBarState < 3 ? getInventoryContents() : getPhoneContents()) + '</div>');
